...
parser.add_option("--arch", dest="arch", help="Unet", default="Nestnet", type="string")############
...
model_path = "./trained_weights/DLA_"+str(model_path_idx)+"/3/"####################
...
class setup_config():
    ...
    def __init__(self, model="Nestnet",########################
                 backbone="RDN",
                 init="random",
...
if config.model == "Unet":
    ...
    raise None
model.compile(optimizer="Adam", #optimizer=Adam(lr=1e-4, decay=5e-4)
              loss=bce_dice_loss, 
